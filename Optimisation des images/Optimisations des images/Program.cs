...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        if (files.Count == 0)
        {
            Console.WriteLine("No images found. Put JPG images in the 'images' folder or pass a path as arg0.");
            return;
        }
    ...
    static void ProcessOneImage(string file, string outputDir, string subfolder)
    {
        ...
        foreach (var (w, h, name) in Targets)
        {
            ...
            var outDir = Path.Combine(outputDir, subfolder);
            Directory.CreateDirectory(outDir);
            var baseName = Path.GetFileNameWithoutExtension(file);
            var outFile = $"{baseName}_{name}.jpg";
            var outPath = Path.Combine(outDir, outFile);

            SaveJpeg(cloned, outPath);
        }
    }

    static void SaveJpeg(Image img, string path)
    {
